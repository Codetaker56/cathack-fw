 ...
 *********************************************************************/
void boot_screen() {
    // CatHack boot logo (real asset: github.com/Stachugit/CatHack images/cathack.png)
    tft.fillScreen(TFT_BLACK);
    tft.setSwapBytes(true);
    tft.pushImage(0, 0, CATBOOT_W, CATBOOT_H, catboot_img);
}
...
void boot_screen_anim() {
    boot_screen();
    int i = millis();
    // still honor a user /boot.jpg|gif on SD/LittleFS or a theme boot_img (stock Bruce behavior)
    int boot_img = 0;
    bool drawn = false;
    if (sdcardMounted) {
    ...
    while (millis() < i + 3000) { // hold logo ~3s
        if (boot_img > 0 && !drawn) {
        ...
        if (check(AnyKeyPress)) { // any key skips the splash
            tft.fillScreen(bruceConfig.bgColor);
            delay(10);
            return;
        }
