...
void USBHIDMouseBase::buttons(uint8_t b) {
    if (b != _buttons) { _buttons = b; }
}

void USBHIDMouseBase::begin() { hid.begin(); }

void USBHIDMouseBase::end() {}

void USBHIDMouseBase::press(uint8_t b) { this->buttons(_buttons | b); }

void USBHIDMouseBase::release(uint8_t b) { this->buttons(_buttons & ~b); }
...
HIDMouseType_t HIDMouseAbs = {
    HID_MOUSE_ABSOLUTE,
    abs_mouse_report_descriptor,
    sizeof(abs_mouse_report_descriptor),
    sizeof(hid_abs_mouse_report_t)
};
...
HIDMouseType_t HIDMouseRel = {
    HID_MOUSE_RELATIVE,
    rel_mouse_report_descriptor,
    sizeof(rel_mouse_report_descriptor),
